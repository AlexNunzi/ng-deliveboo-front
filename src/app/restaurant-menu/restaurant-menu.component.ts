  ...
  implements OnInit, OnDestroy, AfterViewInit
{
  ...
  @Select(HomeState.getRestaurantSelector)
  currentRestaurant$: Observable<Restaurant>;
  @Select(ShopState.getFoodsCart) foodsCart$: Observable<{
    [key: string]: Food;
  }>;
  ...
  ngAfterViewInit(): void {
    ...
    const $modalReference: HTMLElement =
      this.elRef.nativeElement.querySelector('#restaurantModal');

    this.modal = new Modal($modalReference, modalOptions);

    this.cartRestaurantSlug = this.store.selectSnapshot(
      ShopState.getCurrentRestaurantSlug
    );
    if (
      this.cartRestaurantSlug &&
      this.cartRestaurantSlug !== this.restaurantSlug
    ) {
      this.modal.show();
    }
  }
  ...
  deleteCart() {
    this.store.dispatch(new StateReset(ShopState));
  }
}
